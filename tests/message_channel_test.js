...
test("A message can be sent through a port and delivered to its entangled port", 1, function () {
  // var mc = new MessageChannel();
  // mc.port2.addEventListener(function(event) {
    // equal("test", event.data, "We receive the correct message");
    // start();
  // }, false);
  // mc.port1.destinationUrl = window.location.origin;
  // mc.port1.currentTarget = window;
  // mc.port2.start();
