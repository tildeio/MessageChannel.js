  ...
  // if( !window['MessageChannel'] ) {
    ...
    MessagePort.prototype = {
      ...
      close: function() {
        if( this._entangledPortUuid ) {
          this.getEntangledPort()._entangledPortUuid = null;
          this._entangledPortUuid = null;
      ...
      postMessage: function( message ) {
        ...
        var target = this.getEntangledPort();
      ...
      getPort: function( portClone, messageEvent ) {
        ...
        port.getEntangledPort()._entangledPortUuid = port.uuid;
    ...
    MessageChannel.propagateEvent = function( fakeEvent ) {
      ...
      if( fakeEvent.messageChannel ) {
        ...
        for( var i=0 ; i<ports.length ; i++) {
          ports[i].enqueueEvent( fakeEvent );
        }
      }
    };
    ...
    MessageChannel.postMessagePorts = function( source, message, targetOrigin, ports ) {
      ...
      for( var i=0 ; i<ports.length ; i++) {
        entangledPort = ports[i].getEntangledPort();
